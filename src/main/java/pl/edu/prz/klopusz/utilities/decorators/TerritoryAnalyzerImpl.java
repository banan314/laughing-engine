...
public class TerritoryAnalyzerImpl implements TerritoryAnalyzer {
    ...
    Territory whiteTerritory = new TerritoryImpl();
    Territory blackTerritory = new TerritoryImpl();
    ...
    public TerritoryAnalyzerImpl(Board board) {
        this.board = board;
    }
    ...
    public void analyze() {
        ...
        traverseBoard((row, file, state) -> {
            if (state == CellState.SEEDED_WHITE) {
                Square s = new Square(row, file);
                whiteTerritory.add(s);
            } else if (state == CellState.SEEDED_BLACK) {
                Square s = new Square(row, file);
                blackTerritory.add(s);
            }
        });
    }
...
//        return get4Neighbors(file, row);
    ...
    * */
    ...
    protected interface Command {
        void execute(final byte x, final byte y, final CellState state);
    }
    ...
    @Override
    public Territory getWhiteTerritory() {
        return whiteTerritory;
    }

    @Override
    public Territory getBlackTerritory() {
